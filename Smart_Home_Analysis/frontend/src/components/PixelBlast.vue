<template>
  <div ref="containerRef" class="pixel-blast-container" aria-label="PixelBlast interactive background" />
</template>
...
// ---- Touch texture helpers (pure JS, not React-specific) ----
...
export default {
  ...
  props: {
    ...
    edgeFade: { type: Number, default: 0.25 },
  ...
  setup(props) {
    ...
    // Mutable Three.js state — not reactive
    let threeState = null
    let prevReinitCfg = null
    const visibilityRef = { visible: true }
    const speedRef = { current: props.speed }

    const needsReinit = (cfg) => {
      if (!prevReinitCfg) return true
      return prevReinitCfg.antialias !== cfg.antialias ||
        prevReinitCfg.liquid !== cfg.liquid ||
        prevReinitCfg.noiseAmount !== cfg.noiseAmount
    }
    ...
    const initThree = () => {
      ...
      try {
        ...
        console.warn('PixelBlast: WebGL not available, falling back to static background')
        return
      }
      ...
      let raf = 0
      const animate = () => {
        if (props.autoPauseOffscreen && !visibilityRef.visible) {
    ...
    if (typeof document !== 'undefined') {
      visibilityHandler = () => {
        visibilityRef.visible = document.visibilityState === 'visible'
      }
    }
    ...
    // Watch speed separately (stored in a plain ref to avoid re-init)
    watch(() => props.speed, (v) => { speedRef.current = v })

    // Props that just update uniforms (no reinit needed)
    watch(
      () => [props.variant, props.pixelSize, props.color, props.patternScale,
        ...
        props.rippleSpeed, props.rippleThickness, props.rippleIntensityScale,
        props.edgeFade, props.transparent, props.liquidStrength,
        props.liquidRadius, props.liquidWobbleSpeed],
      () => { updateUniforms() }
    )

    // Props that need full reinit
    watch(
      () => [props.antialias, props.liquid, props.noiseAmount],
      () => {
